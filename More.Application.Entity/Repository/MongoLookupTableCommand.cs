...
namespace More.Application.Entity.Repository
{
    ...
    /// </summary>
    public class MongoLookupTableCommand : ITableContextCommand
    {
        ...
        public DataTable ExecuteWithData(params object[] parameters)
        {
            switch (_commandType)
            {
                case LookupTableCommand.GetAllTables:
                    return null;//ExecuteGetAllCommand(parameters);
                case LookupTableCommand.GetTable:
                    return null;//ExecuteGetTableCommand(parameters.Select(p => (SqlParameter)p).ToArray());
                case LookupTableCommand.GetTableByName:
                    return null;//ExecuteGetTableByNameCommand(parameters);
            }
        ...
        //                    schema, tableName, effectiveDate, metadataColumns, keyColumns, dataColumns));
        //        }
        //    }
