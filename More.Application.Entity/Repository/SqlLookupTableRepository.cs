...
namespace More.Application.Entity.Repository
{
    ...
    /// </summary>
    public class SqlLookupTableRepository : IRatingEngineTableRepository
    {
        private readonly Dictionary<string, ILookupTableContext> _contexts = new Dictionary<string, ILookupTableContext>(); 
        ...
        public SqlLookupTableRepository(ILookupTableContext context, DateTime effectiveDate, bool setupMode)
        {
            _contexts.Add("default", context);
            EffectiveDate = effectiveDate;
            SetupMode = setupMode;
        }

        public SqlLookupTableRepository(Dictionary<string, ILookupTableContext> contexts, DateTime effectiveDate, bool setupMode)
        {
            _contexts = contexts;
            EffectiveDate = effectiveDate;
            SetupMode = setupMode;
        ...
        {
            var lookupTables = new List<LookupTableModel>();

            foreach (var context in _contexts)
            {
                DataTable dataTable;
                using (var command = context.Value.GetCommand(LookupTableCommand.GetAllTables))
                {
                    dataTable = command.ExecuteWithData(EffectiveDate);
                }

                var tables = dataTable.Rows.Cast<DataRow>().ToList().Select(TableAdapter.ToLookupTableModel).ToList();
                tables.ForEach(t => t.Properties.Context = context.Key);
                
                lookupTables.AddRange(tables);
            }
        ...
        public virtual void SaveLookupTableRow(LookupTableRowModel model)
        {
            ...
            using (var command = _contexts[lookupTable.Properties.Context].GetCommand(commandType))
            {
        ...
        public virtual void DeleteLookupTableRow(string tableName, string tableId, string rowId)
        {
            ...
            if (row != null)
            {
                ...
                using (var command = _contexts[lookupTable.Properties.Context].GetCommand(LookupTableCommand.DeleteRow))
                {
                    command.Execute(parameters);
                }
            }
            else //insert it with the different effective date (max expiration date)
            {
                ...
                using (var command = _contexts[lookupTable.Properties.Context].GetCommand(LookupTableCommand.InsertRow))
                {
                    command.Execute(parameters);
                }
            }
        ...
        public virtual LookupTable GetLookupTable(string id)
        {
            ...
            foreach (var context in _contexts)
            {
                using (var command = context.Value.GetCommand(LookupTableCommand.GetTable))
                {
                    ...
                    if(!string.IsNullOrEmpty(parameterValues["Metadata"].ToString()))
                        xml = XElement.Parse(parameterValues["Metadata"].ToString());
                    
                    if (table.Rows.Cast<DataRow>().Any() || (xml != null && xml.Element("ColumnMetadata").HasElements))
                    {
                        var lookupTable = TableAdapter.ToLookupTable(parameterValues, table);
                        lookupTable.Properties.Context = context.Key;
        ...
        public virtual void ImportTables(IEnumerable<LookupTable> importTables, bool drop)
        {
            ...
            //process each set of tables for its given context
            var tableGroups = tables.GroupBy(t => t.Properties.Context.ToString());

            foreach (var tableGroup in tableGroups)
            {
                var context = tableGroup.Key;

                if (drop)
                    DropTables(tableGroup, context);

                //create the tables
                using (var command = _contexts[context].GetCommand(LookupTableCommand.CreateTable))
                {
                    ...
                    foreach (var table in tableGroup)
                    {
                        var tableProperties = new Dictionary<string, object>
                        {
                            ...
                            {"ChangeId", Guid.NewGuid().ToString() },
                            {"TableType", table.Properties.GetType().GetProperty("TableType") != null ? table.Properties.TableType : "Custom"},
        ...
        {
            if (dropTables == null)
                throw new ArgumentNullException("dropTables");

            var tables = dropTables as LookupTable[] ?? dropTables.ToArray();

            if (!tables.Any())
                return;

            //process each set of tables for its given context
            var tableGroups = tables.GroupBy(t => t.Properties.Context.ToString()).ToList();
            tableGroups.ForEach(g => DropTables(g, g.Key)); 
        }

        private void DropTables(IEnumerable<LookupTable> tables, string context)
        {
            //physically drop the tables if drop AND setupmode is true; otherwise, soft drop
            using (var command = _contexts[context].GetCommand(LookupTableCommand.DropTable))
            {
                var parameters = tables.Where(t => !string.IsNullOrEmpty(t.Id)).Select(t =>
                    new Dictionary<string, object>
                        {
                            {"Id", new SqlParameter("@Id", t.Id)},
                            {"Permanent", new SqlParameter("@Permanent", true)} //ignoring SetupMode for now
        ...
        public LookupTable GetLookupTable(string name, string schema, DateTime? effectiveDate)
        {
            ...
            foreach (var context in _contexts)
            {
                using (var command = context.Value.GetCommand(LookupTableCommand.GetTableByName))
                {
                    DataTable table;

                    try
                    {
                        table = command.ExecuteWithData(tableNameParameter, schemaNameParameter, effectiveDateParameter,
                        metadataParameter);
                    }
                    ...
                    {
                        var lookupTable = TableAdapter.ToLookupTable(new Dictionary<string, object>
                        {
                            {"Metadata", metadataParameter.Value},
        ...
        public string[] GetConfiguredContexts()
        {
            return _contexts.Select(k => k.Key).ToArray();
        }
