...
namespace NCalc
{
    public class Expression
    {
            ...
            get
            {
                try
                {
                    return _parsedExpression ??
                           (_parsedExpression =
                            Compile(OriginalExpression, (Options & EvaluateOptions.NoCache) == EvaluateOptions.NoCache));
                } catch (Exception exception)
                {
                    ErrorException = exception;
                }
                return _parsedExpression = null;
            }
            private set { _parsedExpression = value; }
        }
        ...
        public object Evaluate()
        {
            if (HasErrors())
            {
                throw new EvaluationException(ErrorException.Message,ErrorException);
            }
